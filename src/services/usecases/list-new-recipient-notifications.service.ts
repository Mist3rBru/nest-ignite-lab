...
@Injectable()
export class ListNewRecipientNotifications implements IListNewRecipientNotifications {
  constructor(
    private readonly findRecipientNotificationsRepository: IFindRecipientNotificationsRepository
  ) {}

  async list(recipientId: string): Promise<IListNewRecipientNotifications.Result> {
    const notifications =
      await this.findRecipientNotificationsRepository.findRecipientNotifications(recipientId)
